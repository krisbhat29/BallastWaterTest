...
#define VERSION_NO            100    // divide by 100
...
void loop() 
{
  if(analogRead(MODE) > VTGTHRESHOLD_CNT) //|| (analogRead(PUMPB_SENSE) < VTGTHRESHOLD_CNT) || (analogRead(PUMPC_SENSE) < VTGTHRESHOLD_CNT) || (analogRead(PUMPD_SENSE) < VTGTHRESHOLD_CNT))
  {
...
//    Serial.println("MANUAL MODE ON\n"); 
  ...
  else
  {
    ...
    for(int i = 2; i<14; i++)
    {
      while(1)
      {
        if((analogRead(READSWITCH) < VTGTHRESHOLD_CNT) || (analogRead(MODE) > VTGTHRESHOLD_CNT))
        {
          break;
        }
      }
